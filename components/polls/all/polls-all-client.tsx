...
interface PollsAllClientProps {
  initialPolls: PollCardData[];
}

export function PollsAllClient({ initialPolls }: PollsAllClientProps) {
  ...
  // Refetch on filter changes (skip initial mount since we have SSR data)
  useEffect(() => {
  ...
  // Load more (called from event handler / observer callback, not from effect)
